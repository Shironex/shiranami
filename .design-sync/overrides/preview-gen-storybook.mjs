...
const COMPOSE = `function compose(S: any, key: string) {
  ...
  const decorated = decorators.reduce((inner: any, dec: any) => () => {
    const out = dec(inner, ctx);
    return out === undefined ? inner() : out;
  }, render);
  // design-sync FORK: run Storybook beforeEach (meta then story) once before the
  // first render. Stories seed Zustand stores / query cache here; stores are
  // exposed on window.<GLOBAL> and story store-imports are shimmed to it
  // (cfg.storyImports.shim), so the seeded instance IS the one the shipped
  // (global) component reads -> the preview renders populated instead of empty.
  const befores: any[] = [meta.beforeEach, st && st.beforeEach].filter((f: any) => typeof f === 'function');
  if (!befores.length) return decorated;
  return function SeededStory() {
    const ranRef = React.useRef(false);
    if (!ranRef.current) {
      ranRef.current = true;
      for (const b of befores) { try { b(ctx); } catch (_e) { /* async/cleanup ignored */ } }
    }
    return decorated();
  };
}`;

...
/// A fresh import carries no analysis at all, and the engine's skip test reads
/// all three measurements in one row read.
#[tokio::test]
...
/// Writing to a deleted id is a no-op, not an error — same contract as the
/// loudness write one screen up.
#[tokio::test]

//! The two clock primitives the workspace needs: `Date.parse` and `Date.now`,
//! as epoch milliseconds.
//!
//! `docs/v2/architecture.md` Appendix B pins no date crate, and adding
//! `chrono`/`time` for a handful of fields would be a workspace-wide dependency
//! decision. This module hand-rolls the small subset instead: ~80 lines, fully
//! tested against `node -e "Date.parse(…)"` in `tests/instant.rs`.
//!
//! **Ported in Phase 4, moved here in Phase 14.** It landed in
//! `shiranami-recommendation::core::instant` because `affinity_score` was the
//! only consumer: that function decays a play by how old it is, so it turns the
//! ISO-8601 `lastPlayedAt` string into an instant and compares it against
//! "now". Phase 4's amendment said to move it down on the second
//! consumer, and Phase 12 produced several — scrobble parks rows against a
//! millisecond clock, the Discord presence card anchors its countdown to one,
//! and lyrics/share validation reads the same timestamps. Recommendation keeps a
//! re-export, so nothing that called it by the old path had to move.
//!

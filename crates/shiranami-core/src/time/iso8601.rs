//! `new Date().toISOString()`, in Rust — the format direction of [`super`].
//!
//! Written in `shiranami-library` in Phase 10 for one caller, its `storage`
//! module's `computedAt`, under a note saying to move it here as soon as a
//! second consumer appeared. Phase 14's `recommendations:*` lane is that
//! consumer: a cached shelf's `generatedAt` is the same
//! `new Date().toISOString()` string, and `shiranami-recommendation` sits beside

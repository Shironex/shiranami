...
import { useScrobbleSection } from './ScrobbleSection.hooks';
 ...
 */
export default function ScrobbleSection() {
  const {
    t,
    status,
    lbToken,
      ...
      >
        ...
        <ProviderRow>
          <div className="flex items-center justify-between gap-2">
            ...
            {status.lastfmConnected ? (
              <Button variant="ghost" size="sm" onClick={onDisconnectLastfm} className="shrink-0">
                <X className="size-4" />
                {t('scrobbleSettings.disconnect')}
              </Button>
            ) : lastfmPendingToken ? (
              <div className="flex shrink-0 items-center gap-2">
                <Button variant="ghost" size="sm" onClick={onCancelLastfm}>
                  {t('scrobbleSettings.cancel')}
                </Button>
                <Button size="sm" onClick={onFinishLastfm} disabled={busy === 'lastfm'}>
                  {busy === 'lastfm' ? (
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            {status.listenBrainzConnected && (
              <Button
                variant="ghost"
                size="sm"
                onClick={onDisconnectListenBrainz}
                className="shrink-0"
              >
                <X className="size-4" />
                {t('scrobbleSettings.disconnect')}
          ...
          {!status.listenBrainzConnected && (
            <>
              ...
              >
                <Input
                  value={lbToken}
                  onChange={e => onLbTokenChange(e.target.value)}

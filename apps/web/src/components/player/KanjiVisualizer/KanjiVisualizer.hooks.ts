...
import type { IKanjiVisualizerProps, IKanjiVisualizerView } from './KanjiVisualizer.types';
...
export function useKanjiVisualizer({
  source,
  active,
}: IKanjiVisualizerProps): IKanjiVisualizerView {
  const colsRef = useRef<IColumn[] | null>(null);
